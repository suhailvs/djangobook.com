...
Create an app by below command::
...
in `urls.py` add::
	...
	urlpatterns = [
		...<other url patterns>...
		url(r'^upload/$', uploader_views.home, name='fileupload'),
...
update `views.py`::
	...
	def home(request):
		if request.method=="POST":
			frm_file = UploadForm(request.POST, request.FILES)		
			if frm_file.is_valid():
				frm_file.save()	
				return HttpResponseRedirect(reverse('fileupload'))
		else:
			frm_file=UploadForm()
		files=Upload.objects.all()
		return render(request,'home.html',{'form':frm_file,'files':files})
...
Create a folder **templates** in folder **uploader**, then create a file **home.html**, ie **sample/uploader/templates/home.html**::
	...
	<div style="padding:40px;margin:40px;border:1px solid #ccc">
		...
		{% for file in files %}
			{{forloop.counter}}.<a href="{{ file.pic.url }}">{{ file.pic.name }}</a>
			({{file.upload_date}})<hr />
		{% endfor %}
	</div>
